...
export class CSVDataBrowserComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.projectService.downloadFile(this.csvFile.blobFileID).subscribe(res => {
      const objectURL = URL.createObjectURL(res);
      // console.log("objectURL", objectURL);
      this.loadCsvData(objectURL);
    },
      error => {
        console.log(error);
      });
  ...
  loadCsvData(objectURL: string): void {
    d3.csv(objectURL, d3.autoType).then(res => {
